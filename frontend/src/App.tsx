...
function useGraphs() {
  ...
  const refresh = useCallback(() => { api.graphs().then(setGraphs).catch(() => setGraphs([])); }, []);
  useEffect(() => { refresh(); const onChanged = () => refresh(); window.addEventListener("knowledge:changed", onChanged); return () => window.removeEventListener("knowledge:changed", onChanged); }, [refresh]);
  return graphs;
}
...
export function App() {
  ...
  const [motionMode, setMotionMode] = useState<"full" | "light" | "static">(() => (localStorage.getItem("lca-motion") as "full" | "light" | "static") || "full");
...
function Materials() {
  ...
  async function finishGeneration(id: string) {
    ...
    motionOrchestrator.emit("knowledge.ready", job.result.id); window.dispatchEvent(new Event("knowledge:changed")); setMessage("结构已保存，正在打开知识脉络。"); navigate(`/knowledge?graph=${job.result.id}`);
  }
...
function Ask() {
  ...
  return <div className="ask-layout"><section className="panel ask-intro"><span className="eyebrow">ASK THE NETWORK</span><h2>问一个问题，<em>沿着证据继续走。</em></h2><p>回答优先来自你的知识库；每个来源都能回到材料和概念。</p><div className="ask-prompt"><textarea value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && (event.metaKey || event.ctrlKey)) submit(); }} placeholder="例如：损失函数为什么会影响泛化？" /><div className="task-actions"><button className="primary-button" onClick={submit} disabled={busy}>{busy ? "检索中…" : "开始探索 ↗"}</button>{busy && <button className="soft-button" onClick={cancel}>取消</button>}{retryId && !busy && <button className="soft-button" onClick={retry}>重试</button>}</div></div><label className="check-option"><input type="checkbox" checked={useDatabase} onChange={(event) => setUseDatabase(event.target.checked)} />使用我的知识库作为主要依据</label><small className="hint">⌘ / Ctrl + Enter 提交</small>{trace.length > 0 && <div className="trace-stream">{trace.slice(-6).map((event, index) => <div className="trace-event" key={`${event.text}-${index}`}><i>{event.type === "tool" ? "◈" : "·"}</i><span>{event.text || "处理中"}</span></div>)}</div>}</section><section className="panel answer-panel">{answer ? <><div className="answer-meta"><span className="status-chip">{answer.has_sources ? "知识库依据" : "需要补充来源"}</span><span>{answer.web_fallback_used ? "含联网补充" : ""}</span></div><h3>{answer.question}</h3><p className="answer-copy">{answer.answer}</p><div className="source-list">{answer.sources.map((source, index) => <SourceCard source={source} index={index} onOpen={(graphId, nodeId) => navigate(`/knowledge?graph=${encodeURIComponent(graphId)}${nodeId ? `&node=${encodeURIComponent(nodeId)}` : ""}&from=ask`)} key={`${source.graph_id || source.url || source.id || "source"}-${index}`} />)}</div></> : <EmptyState text="回答会在这里展开，并保留来源路径。" />}</section></div>;
}

function SourceCard({ source, index, onOpen }: { source: AskResult["sources"][number]; index: number; onOpen: (graphId: string, nodeId?: string) => void }) {
  const content = <><span>{String(index + 1).padStart(2, "0")}</span><div><b>{source.title || source.source_name || "知识来源"}</b><small>{source.snippet || "已定位到相关内容"}</small></div><em>{source.graph_id ? "打开脉络 ↗" : source.url ? "打开网页 ↗" : "已关联来源"}</em></>;
  if (source.url && !source.graph_id) return <a className="source-card" href={source.url} target="_blank" rel="noreferrer">{content}</a>;
  return <button className="source-card" onClick={() => source.graph_id && onOpen(source.graph_id, source.node_id)} disabled={!source.graph_id}>{content}</button>;
}

function Review() {
  const [session, setSession] = useState<{ id: string; mode: string; items: Array<Record<string, unknown>> } | null>(null); const [mode, setMode] = useState<"feynman" | "graph_recall" | "socratic">("feynman"); const [index, setIndex] = useState(0); const [text, setText] = useState(""); const [feedback, setFeedback] = useState(""); const [rating, setRating] = useState<number | null>(null); const [masterySignal, setMasterySignal] = useState<{ mastery?: string; retrievability?: number | null; review_count?: number } | null>(null); const [busy, setBusy] = useState(false);
  const navigate = useNavigate();
  const current = session?.items[index];
  const context = reviewContext(current);
  async function start() { setBusy(true); try { const next = await api.reviewSession(mode); setSession(next); setIndex(0); setText(""); setFeedback(""); setRating(null); setMasterySignal(null); } catch (error) { setFeedback(error instanceof Error ? error.message : "暂时无法开始练习"); } finally { setBusy(false); } }
  async function submit() { if (!session || !current || !text.trim()) return; setBusy(true); try { const result = await api.answerReview(session.id, { item_id: current.id, text }); setFeedback(result.feedback); setRating(null); } catch (error) { setFeedback(error instanceof Error ? error.message : "提交失败"); } finally { setBusy(false); } }
  async function rate(value: number) { if (!session || !current) return; setBusy(true); try { const result = await api.answerReview(session.id, { item_id: current.id, text, rating: value }); const latest = context?.conceptId ? await api.galaxy().catch(() => null) : null; const concept = latest?.concepts.find((item) => item.id === context?.conceptId); setMasterySignal(concept?.mastery || null); setFeedback(result.feedback); setRating(value); motionOrchestrator.emit("review.persisted", current.id as string); } catch (error) { setFeedback(error instanceof Error ? error.message : "评分失败"); } finally { setBusy(false); } }
  return <div className="review-layout"><section className="panel review-hero"><span className="eyebrow">UNDERSTANDING STUDIO</span><h2>今天不背答案，<em>练习解释。</em></h2><p>题目从你的知识关系中生成，反馈会指出缺失的条件、关系和证据。</p>{current ? <><span className="status-chip">第 {index + 1} / {session?.items.length} 题 · {session?.mode}</span>{context && <div className="review-context"><div><small>当前概念</small><b>{context.label || context.conceptId}</b><span>{context.graphTitle || "已从知识空间接入"}</span></div><div className="review-context-actions"><button className="soft-button" onClick={() => navigate(`/galaxy?concept=${encodeURIComponent(context.conceptId)}&focus=${encodeURIComponent(context.conceptId)}&zoom=1.05&from=review`)}>在星图中查看 ↗</button>{context.graphId && context.nodeId && <button className="soft-button" onClick={() => navigate(`/knowledge?graph=${encodeURIComponent(context.graphId)}&node=${encodeURIComponent(context.nodeId)}&from=review`)}>回到脉络 ↩</button>}</div></div>}<h3 className="review-question">{String(current.prompt || current.question || "请解释这条知识关系。")}</h3><textarea className="review-answer" value={text} onChange={(event) => setText(event.target.value)} placeholder="先写出你的理解，再补充条件、例子或证据…" /><div className="review-controls"><button className="primary-button" onClick={submit} disabled={busy || !text.trim()}>{busy ? "记录中…" : "提交理解 ↗"}</button>{feedback && <span className="feedback-line">{feedback}</span>}</div>{feedback && rating === null && <div className="rating-controls" aria-label="为本题评分"><span>这次理解到哪一步？</span>{[[0, "忘记"], [1, "困难"], [2, "模糊"], [3, "掌握"]].map(([value, label]) => <button key={value} className="soft-button" onClick={() => rate(value as number)} disabled={busy}>{label}</button>)}</div>}</> : <><div className="review-mode-picker" aria-label="选择理解方式"><span>选择练习方式</span>{[["feynman", "费曼解释"], ["graph_recall", "关系回忆"], ["socratic", "苏格拉底追问"]].map(([value, label]) => <button key={value} className={`soft-button ${mode === value ? "is-selected" : ""}`} onClick={() => setMode(value as typeof mode)}>{label}</button>)}</div><button className="primary-button" onClick={start} disabled={busy}>{busy ? "准备中…" : "开始一组理解练习 ↗"}</button></>}</section><section className="panel review-map"><span className="eyebrow">LEARNING SIGNAL</span><h3>掌握度应该回到知识空间</h3><div className="signal-ring"><strong>{session ? `${index + 1}` : "—"}</strong><span>{session ? "正在形成理解信号" : "完成练习后显示"}</span></div>{masterySignal && <div className="mastery-readout"><small>评分已同步到星图</small><strong>{masterySignal.mastery || "已记录"}</strong><span>{typeof masterySignal.retrievability === "number" ? `可提取度 ${Math.round(masterySignal.retrievability * 100)}%` : "掌握信号已保存"} · {masterySignal.review_count || 0} 次复习</span></div>}<p>答题后的概念状态、薄弱关系和下一步复习都会在星图中保留。</p>{rating !== null && session && index < session.items.length - 1 && <button className="soft-button" onClick={() => { setIndex(index + 1); setText(""); setFeedback(""); setRating(null); setMasterySignal(null); }}>下一题 →</button>}</section></div>;
}

function reviewContext(item?: Record<string, unknown>) {
  if (!item) return null;
  const meta = item.meta && typeof item.meta === "object" ? item.meta as Record<string, unknown> : {};
  const value = (key: string) => typeof item[key] === "string" && item[key] ? String(item[key]) : "";
  const metaValue = (key: string) => typeof meta[key] === "string" && meta[key] ? String(meta[key]) : "";
  const conceptId = value("concept_id") || metaValue("concept_id");
  if (!conceptId) return null;
  return {
    conceptId,
    label: value("concept_label") || value("concept") || metaValue("node_label"),
    graphId: value("graph_id") || metaValue("graph_id"),
    nodeId: value("node_id") || metaValue("node_id"),
    graphTitle: value("graph_title") || metaValue("graph_title"),
  };
}
