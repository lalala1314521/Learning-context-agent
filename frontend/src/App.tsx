...
function Review() {
  const [session, setSession] = useState<{ id: string; mode: string; items: Array<Record<string, unknown>> } | null>(null); const [mode, setMode] = useState<"feynman" | "graph_recall" | "socratic">("feynman"); const [index, setIndex] = useState(0); const [text, setText] = useState(""); const [feedback, setFeedback] = useState(""); const [rating, setRating] = useState<number | null>(null); const [busy, setBusy] = useState(false);
  const navigate = useNavigate();
  async function start() { setBusy(true); try { const next = await api.reviewSession(mode); setSession(next); setIndex(0); setText(""); setFeedback(""); setRating(null); } catch (error) { setFeedback(error instanceof Error ? error.message : "暂时无法开始练习"); } finally { setBusy(false); } }
