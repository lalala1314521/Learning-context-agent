...
type Point = { id: string; label: string; domainId?: string | null; x: number; y: number; radius: number; color: number; mastery?: string; labelVisible: boolean; labelX?: number; labelY?: number };
  ...
  useEffect(() => {
    ...
    app.init({ resizeTo: root, backgroundAlpha: 0, antialias: true, resolution: Math.min(window.devicePixelRatio || 1, 2) }).then(() => {
      ...
      points.forEach((point) => {
        ...
        const masteryColor = masteryTone(point.mastery);
        const masteryAlpha = masteryOpacity(point.mastery);
        const star = new Graphics().circle(point.x, point.y, point.radius).fill({ color: point.color, alpha: active ? masteryAlpha : .16 });
        star.circle(point.x, point.y, point.radius + 7).stroke({ color: masteryColor, alpha: active ? .2 : .03, width: 1 });
        star.eventMode = "static"; star.cursor = "pointer"; star.on("pointertap", () => onSelect(concept)); starLayer.addChild(star);
        if (concept.mastery?.mastery && !concept.id.startsWith("domain:")) {
          const masteryRing = new Graphics().circle(point.x, point.y, point.radius + 10).stroke({ color: masteryColor, alpha: active ? .46 : .08, width: 1.2 });
          starLayer.addChild(masteryRing);
        }
