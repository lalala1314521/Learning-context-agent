...
export function GalaxyScene() {
  ...
  useEffect(() => { api.galaxy().then((payload) => { setData(payload); const target = payload.concepts.find((item) => item.id === requestedConcept || item.canonical_label === requestedConcept); if (target) { setSelected(target); setFocusId(target.id); setZoom(1.05); motionOrchestrator.emit("concept.focused", target.id); } motionOrchestrator.emit("scene.entered", "galaxy"); }).catch(() => setData({ concepts: [], links: [], domains: [] })); }, [requestedConcept]);
