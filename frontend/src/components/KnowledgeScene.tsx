...
export function KnowledgeScene({ graphs }: { graphs: Graph[] }) {
  ...
  return <div className="knowledge-layout"><section className="panel graph-panel">{entrySource && <div className="scene-handoff"><span className="scene-handoff-beam" /><div><small>{entrySource === "ask" ? "问答来源接力" : "概念接力"}</small><b>{selected?.label || "正在定位知识点"}</b></div><em>已保留上下文</em></div>}<div className="section-heading"><div><span className="eyebrow">KNOWLEDGE THREAD / {detail?.graph.title || "加载中"}</span><h2>主线与关系</h2></div><span className="status-chip">{detail?.nodes.length || 0} 个知识点</span></div>{detail ? <GraphCanvas nodes={detail.nodes} links={structureEdges} selected={selected?.id} onSelect={handleSelect} /> : <div className="scene-loading">正在还原结构…</div>}</section><aside className="panel inspector"><span className="eyebrow">INSPECTOR</span>{selected ? <><h3>{selected.label}</h3><span className="type-pill">{selected.node_type || "concept"}</span><p>{selected.note || "这条知识点还没有补充说明。"}</p><div className="evidence-box"><small>原文依据</small><b>{detail?.graph.source_name || "当前材料"}</b><span>{selected.evidence || "当前节点暂无独立证据摘录。"}</span></div><Link className="soft-button" to={`/galaxy?concept=${encodeURIComponent(selected.concept_id || selected.label)}&from=knowledge&graph=${encodeURIComponent(graphId || "")}`}>在星图中定位 ↗</Link></> : <EmptyInspector />}</aside></div>;
}
