...
- 星图使用 PixiJS 自定义七层场景：背景、领域光场、关系轨迹、领域轮廓、概念星体、重点标签和焦点脉冲层；概念从真实 `/galaxy` 数据读取，不再使用随机节点。
- 星图增加概念搜索、匹配数量、镜头缩放和复位控制；搜索结果会收窄到当前局部关系，缩放不会重新请求或随机重排数据。
- 星图搜索支持 `/` 快捷聚焦、`Enter` 选择首个结果、`Escape` 清除；选中概念后会把一跳邻域提亮并压低无关星体，手动“展开邻域”不会重新随机布局。
...
- 概念路径过滤已拒绝关系，按关系类型区分有向边与对称边，并把关系类型、置信度和证据随路径返回；联网正文标签改为中性“网页正文”。
