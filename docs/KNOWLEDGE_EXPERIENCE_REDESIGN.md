...
## 18. 阶段 3 增量：星图总览层（2026-09-19）

星图已从默认概念平铺推进为两级阅读：0.85 倍镜头显示由真实概念归属聚合的领域节点与领域间聚合边；放大后显示概念、真实关系和局部邻域；概念深链直接进入概念层；搜索自动进入概念层。该增量降低了 500+ 概念首屏的拥挤度，同时保留从总览到细节的连续入口。

尚未关闭的星图任务：标签碰撞避让、选边后的证据阅读、相机连续过渡、Pixi 场景在选择变化时的局部更新与资源复用。

## 19. 阶段 3 增量：标签与关系证据阅读（2026-09-19）

概念层新增标签碰撞过滤与边界约束，避免高频标签相互覆盖或离开画布；概念详情新增真实关系清单，关系卡显示关系类型和状态，选中后回显独立证据摘录，并向全局动效编排发送关系选择事件。

下一步仍需处理 Pixi 画布的局部更新、镜头连续过渡、关系路径动画和更大数据量下的性能曲线。

## 20. 阶段 3 增量：局部选择更新与连续镜头（2026-09-19）

为避免星图交互在大数据量下反复创建 Pixi 资源，本轮将场景生命周期拆成两部分：数据、领域层级和显式邻域焦点变化时重建七层场景；选择概念时只清理并重绘 L6 焦点层，关系证据保持在 DOM Inspector 中阅读。镜头不再把每个缩放值作为场景重建条件，而是通过目标缩放引用和 ticker 的内插逐帧逼近目标；减少动画模式跳过插值并直接设置目标值。场景卸载时统一销毁应用、子对象和选择引用，确保路由切换后没有残留渲染循环。

本轮实现的交互约束如下：

- 点击概念只改变选中脉冲与右侧关系状态；需要展开真实一跳邻域时由“展开邻域”动作显式触发，避免误把每次点选变成昂贵布局操作。
- 默认 0.85 倍领域总览和概念层之间仍以层级阈值切换；同一层级内的连续缩放沿现有场景平滑过渡。
- `prefers-reduced-motion` 或静态模式下不运行脉冲与相机插值，但仍保留概念选择、关系证据和重置镜头等功能。

验证结果：TypeScript 检查通过；Vite 生产构建通过；真实浏览器中默认领域总览、放大后的 523 个概念、LangSmith 深链、6 条真实关系及关系证据均可见。后续仍需补关系路径的方向性演绎、跨页面概念共享元素，以及 500/2,000/10,000 节点档位的帧率和资源曲线，不能用当前单库回归代替压力验收。

## 21. 阶段 3 增量：关系方向演绎与全局 UI 风格（2026-09-19）

关系阅读继续从“静态连线”推进为可追踪的语义路径：每条真实边沿 `from → to` 绘制方向箭头；Inspector 选边后，星图在对应三次贝塞尔曲线上显示高亮轨迹和移动光点。光点只存在于选中边，避免所有边同时运动造成噪声；静态模式仍保留箭头、颜色和证据关系，但关闭 ticker 的移动更新。

全局视觉层新增四类低干扰材质：

- aurora 背景与网格星尘，给页面增加空间深度，不参与交互命中；
- live/status 呼吸，表达知识网络和当前镜头仍处于可探索状态；
- 卡片 hover/selected 的边缘光带，把鼠标焦点与证据阅读顺序连接起来；
- 导航图标、最近脉络节点和星图图例的微动效，统一全站节奏。

上述层都通过 `data-motion=full|light|static` 控制。完整模式保持路径演绎，轻量模式降低背景频率和透明度，静态模式停止背景、状态和路径移动，仍保留点击、缩放、证据和路由功能。

验证结果：浏览器完整模式可见关系方向箭头、选中路径和移动光点；工作台可见新的背景材质与卡片层次；无控制台错误，生产构建与 TypeScript 检查通过。下一阶段应继续完成问答→星图→脉络的共享概念载体、四条完整动效分镜和分档压力性能记录。

## 22. 阶段 4 增量：跨场景概念接力（2026-09-19）

跨页面跳转从“只带一个 graph 参数”升级为可恢复的概念上下文：

1. 问答来源卡将 `graph_id`、`node_id` 组合成知识脉络深链，并标记 `from=ask`。
2. 知识脉络进入时按节点 ID、concept ID、标签三种稳定身份匹配，避免后端返回顺序变化导致默认选错节点。
3. 知识脉络到星图的链接继续携带 `concept`、`graph` 和 `from=knowledge`，星图直接进入概念层并聚焦目标。
4. App 的场景 key 同时包含 pathname 与 search，同一路由的概念切换也能触发统一进出场动画。

目标场景显示短暂的接力条，表达“问答来源接力”“知识脉络接力”和目标概念；提示不会代替 Inspector、证据卡或星图操作，静态模式仅移除位移动效。这样四条分镜中的“回答概念 → 星图对焦 → 邻域 → 返回脉络”拥有了可验证的身份链，而不是靠组件内部猜测当前选中项。

真实浏览器验证已覆盖：问答来源式知识脉络深链定位 LangSmith；从该节点进入星图后 URL 保留上下文，LangSmith 选中、6 条关系和证据阅读均可见，控制台无错误。后续还需要将问答任务实际返回的 `node_id` 覆盖进端到端录制，并补充返回动作的镜头恢复和 500/2,000/10,000 节点性能记录。

## 23. 阶段 4 增量：返回脉络与镜头恢复（2026-09-19）

为完成“回答概念 → 星图对焦 → 邻域 → 返回”的可操作闭环，知识脉络链接到星图时携带：

- `concept`：稳定概念身份，用于加载后聚焦；
- `focus`：恢复同一概念的邻域焦点；
- `zoom`：恢复进入星图时的镜头层级；
- `graph`：记录返回目标脉络；
- `from`：标记接力来源，驱动短暂上下文提示。

星图 Inspector 的“返回脉络”使用同一概念 ID 回链，知识脉络同时接受节点 ID、concept ID 和标签匹配，因此旧数据、节点排序变化或跨图概念仍能落到可读节点。返回动作使用 React Router 内部转场，保留全局场景动画和上下文提示。

真实浏览器验证：LangSmith 从知识脉络进入星图后恢复 `focus` 与 `zoom`；点击返回后回到对应脉络，16 个节点出现并重新选中 LangSmith，控制台无错误。下一阶段继续录制四条分镜并测量不同规模下的镜头、路径和资源曲线。

## 24. 阶段 4 增量：理解练习接力（2026-09-19）

复习题不再是与知识空间隔离的答题表单。服务层给 Feynman、Socratic 和闪卡题目补齐 `concept_id`、`graph_id`、`node_id`、`concept_label`，同时在 `meta` 保留同一组身份，前端可以兼容旧题型和跨版本会话。题目生成仍以解释、关系、条件和证据为核心，身份字段只承担导航与状态回写，不混入提示文本。

复习页新增“当前概念”上下文卡：

- “在星图中查看”把概念 ID、焦点和 1.05 倍镜头写入 URL，并以 `from=review` 触发短暂接力提示；
- “回到脉络”在题目具备图和节点身份时回到原材料，并用节点 ID/概念 ID恢复选中；
- 图回忆等没有单一概念的题型不显示虚假的概念入口，仍保留原题型交互；
- 评分完成后仍由服务端写入 FSRS 与复习历史，导航不会替代持久化确认。

这一步关闭分镜 D 的“理解练习 → 掌握信号 → 概念空间”入口缺口。下一阶段需要继续把评分后的 mastery/retrievability 直接回显到星图节点，并对多题连续练习、刷新恢复和 500/2,000/10,000 节点压力曲线进行验收。

## 25. 阶段 4 增量：掌握信号回流（2026-09-19）

评分成功后，复习页不再停留在“已记录”文字。前端等待 `answerReview` 成功，再调用星图数据接口按 `concept_id` 找到服务端最新的 `mastery`、`retrievability` 和 `review_count`，在右侧学习信号面板展示：

- 掌握等级（如 `strong`、`medium`、`weak`）；
- 可提取度百分比，用于解释下一次复习压力；
- 已完成的复习次数；
- 下一题按钮继续沿用原会话，不重置已写入的状态。

这让“评分 → FSRS 调度 → 掌握信号 → 星图节点”的因果链在同一页面可见，也避免用乐观 UI 代替服务端确认。下一步需把该信号在星图节点视觉编码中做成可读图例，并验证刷新、断网和多题连续会话下的降级状态。

## 26. 阶段 3/4 增量：星图掌握状态视觉编码（2026-09-19）

星图节点外环现在读取 `concept.mastery.mastery` 并使用固定色阶表达学习状态：青绿色代表 `strong`，暖黄色代表 `medium`，橙色代表 `weak`，粉红色代表 `forgotten`，未复习概念使用蓝色基础光。节点填充仍保持领域颜色，关系线仍保持真实关系颜色；掌握状态只作用于节点外环和透明度，用户可以同时读出“它属于哪个领域”“它与谁有关系”“我对它掌握到什么程度”。

图例增加“掌握/待巩固”状态提示，Inspector 增加可提取度读数，复习评分后的数据刷新会立即反映到星图。静态动效模式仍保留色阶和图例，不依赖 ticker。下一步继续处理多题会话刷新恢复、断网降级提示，以及 500/2,000/10,000 节点下的状态编码性能。
